...
SELECT P.CodProdotto,
	   ...
	   P.Categoria
FROM Prodotto P,Certifica C, Scontrino S
WHERE C.Prodotto=P.CodProdotto AND 
	  C.Scontrino=S.Id AND 
	  S.Iscritto=1;
...
SELECT P.Categoria, 
	   SUM(S.SubTotale) AS Guadagno_Max
FROM Prodotto P, 
	 Categoria C, 
	 Certifica CE, 
	 Scontrino S
WHERE P.Categoria=C.NomeCategoria AND 
...
FROM Dipendente D, 
	 ...
	 Certifica CE, 
	 Scontrino S
WHERE D.Categoria=C.NomeCategoria AND 
...
SELECT I.CodIscritto,
	   I.Nome,
	   I.Cognome,
	   MAX(SC.Livello) AS Livello_massimo
FROM Iscritto I, 
	 Sconto SC, 
	 Scontrino S, 
	 Scaglione SCA,
...
SELECT COUNT(S.Id) AS Numero_Acquisti,
	   C.NomeCategoria, 
	   MAX(SC.Livello) AS Livello_attuale
FROM Scontrino S,
	 Categoria C,
	 Sconto SC,
	 Iscritto I, 
